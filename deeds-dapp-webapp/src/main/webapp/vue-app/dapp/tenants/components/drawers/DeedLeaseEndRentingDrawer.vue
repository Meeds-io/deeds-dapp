...
<template>
  <deeds-drawer
    ...
    @closed="$emit('closed')">
    <template #title>
      <h4 class="text-capitalize">{{ $t('deedsRentEndRentingTitle', {0: cardType, 1: nftId}) }}</h4>
    </template>
...
export default {
  ...
  created() {
    this.$root.$on('deed-end-renting-open', this.open);
  },
  beforeDestroy() {
    this.$root.$off('deed-end-renting-open', this.open);
  },
  methods: {
    open(lease) {
      this.lease = lease;
