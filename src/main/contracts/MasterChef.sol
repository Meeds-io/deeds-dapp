...
contract MasterChef is Ownable, FundDistribution {
    ...
    event ClaimReward(address indexed user, address indexed lpAddress, uint256 amount);
    ...
    function setMeedPerMinute(uint256 _meedPerMinute) public onlyOwner {
    ...
    function addFund(address _fundAddress, uint256 _value, bool _fixedPercentage, bool _isLPToken) public onlyOwner {
    ...
    function setFundAllocation(address _lpAddress, uint256 _value, bool _fixedPercentage) public onlyOwner {
        FundInfo storage fund = fundInfos[_lpAddress];
        if (_fixedPercentage) {
    ...
    function updateAllFundRewards() public {
    ...
    function batchUpdateFundRewards(address[] memory _fundAddresses) public {
    ...
    function updateFundReward(address _lpAddress) public {
        ...
        if (fund.isLPToken) {
          ...
          _mint(_lpAddress, meedRewardAmount);
        }
