...
public class WebSecurityConfig {
  ...
  public SecurityFilterChain filterChain(HttpSecurity http, DeedAuthenticationProvider authProvider) throws Exception {
    http
        .authorizeRequests(authorizeRequests -> authorizeRequests.requestMatchers("/static/**", "/api/deeds/**", "/api/hubs/**").permitAll())
        .authenticationProvider(authProvider)
        .csrf(csrf -> {
          csrfTokenRepository.setCookiePath("/");
          csrfTokenRepository.setCookieHttpOnly(false);
          csrf.csrfTokenRepository(csrfTokenRepository);
          csrf.ignoringRequestMatchers("/static/**", "/api/deeds/**", "/api/hubs/**");
        })
        .headers(headers -> {
