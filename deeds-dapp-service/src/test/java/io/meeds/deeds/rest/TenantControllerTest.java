...
@SpringBootTest(classes = { TenantController.class, DeedAuthenticationProvider.class, WebSecurityConfig.class })
@AutoConfigureWebMvc
@AutoConfigureMockMvc(addFilters = false)
class TenantControllerTest {

  private static final int      NFT_ID   = 3;
  ...
  void whenAuthenticatedGetDeedTenant_thenNOk() throws Exception {
    when(tenantService.getDeedTenant(USERNAME, NFT_ID)).thenReturn(new DeedTenant());
  ...
  void whenAnonymousGetDeedTenantStartTile_thenNOk() throws Exception {
    mockMvc.perform(get("/api/tenant/" + NFT_ID + "/startDate")).andExpect(status().isOk());
  }
