...
public class WomService {
  ...
  public WomConnectionResponse connectToWom(WomConnectionRequest hubConnectionRequest) throws WomException {
    LOG.info("Hub connection {} initiation for owner {} with Deed Id {} to the WoM",
             hubConnectionRequest.getAddress(),
             hubConnectionRequest.getHubOwnerAddress(),
  ...
  public String getDeedOwnerAddress(long deedId, boolean updateTransaction, String potentialDeedOwner) { // NOSONAR
    ...
    DeedTenant deedTenant = tenantService.getDeedTenantOrImport(deedId);
    Hub hub = getHub(deedId);
    String ownerAddress = lease == null ? deedTenant.getOwnerAddress() : lease.getOwnerAddress();

    if (StringUtils.isBlank(ownerAddress) || StringUtils.equalsIgnoreCase(ownerAddress, hub.getDeedOwnerAddress())) {
      return hub.getDeedOwnerAddress();
    } else {
      ownerAddress = Arrays.asList(potentialDeedOwner,
                                   deedTenant.getOwnerAddress(),
  ...
  public String getDeedManagerAddress(long deedId, // NOSONAR
    ...
    DeedTenant deedTenant = tenantService.getDeedTenantOrImport(deedId);
    Hub hub = getHub(deedId);
    String managerAddress = lease == null ? deedTenant.getOwnerAddress() : lease.getManagerAddress();

    if (StringUtils.isBlank(managerAddress) || StringUtils.equalsIgnoreCase(managerAddress, hub.getDeedManagerAddress())) {
      return hub.getDeedManagerAddress();
    } else {
      managerAddress = Arrays.asList(potentialManagerAddress,
